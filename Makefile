...
CSI_SPEC := spec.md
CSI_PROTO := volumegroup.proto
## Build go language bindings
CSI_A := volumegroup.a
CSI_GO := lib/go/volumegroup/volumegroup.pb.go
CSI_PKG := lib/go/volumegroup
...
$(CSI_GO): $(CSI_PROTO)
	$(MAKE) -C lib/go volumegroup/volumegroup.pb.go
...
check: $(CSI_PROTO)
	awk '{ if (length > 75) print NR, $$0 }' $? | diff - /dev/null
